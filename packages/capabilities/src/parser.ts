 ...
 */
export interface ManifestFragments {
  manifestPath: string;
  name?: string;
  description?: string;
...
/**
 * A manifest file validated for running: fragments plus the two things a
 * harness needs. The runtime's `AgentManifest` (which also admits SDK
 * instance forms) is a widening of this shape.
 */
export type AgentManifestFile = ManifestFragments & {
  name: string;
  harness: HarnessSpec;
};
