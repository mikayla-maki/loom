...
export function ttyPermissionHandler(opts: {
  out?: NodeJS.WritableStream;
  in?: NodeJS.ReadableStream;
} = {}): PermissionHandler {
  const out = opts.out ?? process.stderr;
  const inp = (opts.in ?? process.stdin) as NodeJS.ReadableStream & { isTTY?: boolean };
  return async (req: PermissionRequest) => {
    ...
    const rl = readline.createInterface({ input: inp as NodeJS.ReadableStream, output: out });
    ...
    if (answer === "a" || answer === "allow" || answer === "y") decision = "allow_once";
...
function formatRequest(req: PermissionRequest): string {
  ...
  if (req.newCapabilities) lines.push(`new capabilities: ${JSON.stringify(req.newCapabilities)}`);
