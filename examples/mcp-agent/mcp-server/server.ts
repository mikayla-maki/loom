...
const DOCUMENTS: Record<string, { title: string; body: string }> = {
  welcome: {
    title: "Welcome",
    body:
      "Welcome to the Loom MCP example. This document is read by the " +
  ...
  changelog: {
    title: "Changelog",
    body:
      "- v0.1.0  Initial example MCP server.\n" +
      "- v0.0.1  Pre-history; nothing of note.",
  },
  roadmap: {
    title: "Roadmap",
    body:
      "Future work: HTTP transport, MCP resources & prompts, " +
...
server.registerTool(
  "read_document",
  {
    title: "Read document",
    description:
      "Read one of the mock documents by id. Returns the document's " +
...
server.registerTool(
  "query_table",
  {
    title: "Query table",
    description:
      "Return up to `limit` rows from the named mock table. Available " +
      'tables: "users", "orders".',
    inputSchema: {
...
server.registerTool(
  "set_status",
  {
    title: "Set status",
    description:
      "Set the user's presence status. The MCP server accepts any " +
...
server.registerTool(
  "send_alert",
  {
    title: "Send alert",
    description:
      "Send an alert message to a channel. Requires MOCK_API_KEY in the " +
        ...
        content: [
          {
            type: "text",
            text:
              "MOCK_API_KEY missing from server env. Configure it via " +
      ...
      content: [
        {
          type: "text",
          text:
            `[mock alert] channel=${channel} message=${JSON.stringify(message)} ` +
...
server.registerTool(
  "dangerous_delete",
  {
    ...
    description:
      "Pretend to delete a path. Deliberately omitted from the demo " +
